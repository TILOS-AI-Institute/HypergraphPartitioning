...
spec_iters_list = [1, 2] #, 3, 4, 5] # number of iterations for the spectral framework
best_solns_list = [5] # number of best partition solutions used for cut elimination
seed_list = [0] # random seed

work_dir = pwd()
work_dir *= "/ablation_hmetis_rpt_julia_titan_v2/"
                            ...
                            for spec_iters in spec_iters_list
                                for UBfactor in UBfactor_list
                                    for design in design_list
                                        ...
                                        run(`sh -c $cmd`, wait=true)
                                        t = @elapsed cutsize, hmetis_cut = Main.SpectralRefinement.SpectralHmetisRefinement(hg=hypergraph_file, pfile=initial_solution_file, Nparts = Nparts, 
