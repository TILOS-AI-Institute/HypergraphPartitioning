...
work_dir = pwd()
work_dir *= "/hmetis_rpt_julia_ibm_weighted_latest/"

if isdir(work_dir) == false
    mkdir(work_dir)
end

result_file = work_dir * partitioner * "_" * benchmark * "_summary_file_standalone.txt"
...
design_list = String[]
benchmark_dir = "/home/bodhi91/sandbox/Partitioners/SpecPart-Results/benchmark/ISPD_weight_benchmark/"
                            ...
                            for spec_iters in spec_iters_list
                                for UBfactor in UBfactor_list
                                    for design in design_list
                                        ...
                                        #run(`sh -c $cmd`, wait=true)
                                        t = @elapsed cutsize, hmetis_cut = Main.SpectralRefinement.SpectralHmetisRefinement(hg=hypergraph_file, Nparts = Nparts, 
