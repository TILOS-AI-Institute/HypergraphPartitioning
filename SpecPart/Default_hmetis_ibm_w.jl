...
seed_list = Vector{Int}(0:20)

work_dir = pwd()
work_dir *= "/default_hmetis_rpt_julia_ibm_w_latest/"
    ...
    for Nparts in Nparts_list
        for UBfactor in UBfactor_list
            for seed in seed_list
                ...
                run(`sh -c $cmd`, wait=true)
                t = @elapsed run(`$hmetis_exe $hypergraph_file "" $Nparts $UBfactor 1 1 1 0 1 0 $seed`, wait=true)
